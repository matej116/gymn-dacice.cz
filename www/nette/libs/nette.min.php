...
new
Exception('Nette Framework requires PHP 5.2.0 or newer.');}error_reporting(E_ALL|E_STRICT);@set_magic_quotes_runtime(FALSE);iconv_set_encoding('internal_encoding','UTF-8');extension_loaded('mbstring')&&mb_internal_encoding('UTF-8');umask(0);@header('X-Powered-By: Nette Framework');@header('Content-Type: text/html; charset=utf-8');empty($GLOBALS[0])&&$GLOBALS[0]=array();define('NETTE',TRUE);define('NETTE_DIR',dirname(__FILE__));define('NETTE_VERSION_ID',20012);define('NETTE_PACKAGE','PHP 5.2');interface
IBarPanel{function
getTab();function
getPanel();}interface
IPresenter{function
...
constructUrl(PresenterRequest$appRequest,Url$refUrl);}interface
IFreezable{function
freeze();function
isFrozen();}interface
IComponent{const
...
setConnection(Connection$connection);}interface
...
nodeClosed(MacroNode$node);}interface
...
__construct($message,$code,$severity,$file,$line,$context,Exception$previous=NULL){if(PHP_VERSION_ID<50300){$this->previous=$previous;parent::__construct($message,$code);}else{parent::__construct($message,$code,$previous);}$this->severity=$severity;$this->file=$file;$this->line=$line;$this->context=$context;}function
...
__unset($name){ObjectMixin::remove($this,$name);}}final
class
...
function
load(){if(func_num_args()>1){self::$vars=func_get_arg(1);if(self::$vars===TRUE){return require func_get_arg(0);}extract(self::$vars);}return require func_get_arg(0);}}abstract
class
AutoLoader
extends
Object{static
...
Html::el('span')->setText($file);}}static
function
...
NULL;}if($severity===E_RECOVERABLE_ERROR||$severity===E_USER_ERROR){if(DebugHelpers::findTrace(PHP_VERSION_ID<50205?debug_backtrace():debug_backtrace(FALSE),'*::__toString')){$previous=isset($context['e'])&&$context['e']instanceof
...
function
barDump($var,$title=NULL){if(!self::$productionMode){$dump=array();foreach((is_array($var)?$var:array(''=>$var))as$key=>$val){$dump[$key]=DebugHelpers::clickableDump($val);}self::$dumpPanel->data[]=array('title'=>$title,'dump'=>$dump);}return$var;}static
function
...
Strings::fixEncoding($var);}elseif(is_array($var)){static$marker;if($marker===NULL){$marker=uniqid("\x00",TRUE);}if(isset($var[$marker])){return"\xE2\x80\xA6RECURSION\xE2\x80\xA6";}elseif($level<Debugger::$maxDepth||!Debugger::$maxDepth){$var[$marker]=TRUE;$res=array();foreach($var
...
DebugBlueScreen
extends
Object{private$panels=array();function
...
foreach($_SESSION
as$k=>$v)echo'<tr><th>',htmlspecialchars($k),'</th><td>',$k==='__NF'?'<i>Nette Session</i>':DebugHelpers::clickableDump($v),"</td></tr>\n";?>
			</table>
			<?php endif?>
			</div>
...
foreach($_SESSION['__NF']['DATA']as$k=>$v)echo'<tr><th>',htmlspecialchars($k),'</th><td>',DebugHelpers::clickableDump($v),"</td></tr>\n";?>
			</table>
			</div>
			<?php endif?>
...
json_encode(Strings::fixEncoding($output))?>;
...
2?'nette-alt':''?>">
	...
	<td><pre><?php echo
htmlspecialchars(Strings::fixEncoding($message)),' in ',DebugHelpers::editorLink($file,$line),':',$line?></pre></td>
</tr>
<?php endforeach?>
</table>
</div>
...
instanceof
ForwardResponse){$request=$response->getRequest();continue;}elseif($response
instanceof
...
function
initializePanel(Application$application){Debugger::$blueScreen->addPanel(create_function('$e','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('application'=>$application)).'-1], EXTR_REFS);
			return $e ? NULL : array(
...
instanceof
Route?$router->getMask():NULL,'request'=>$request,'module'=>rtrim($module,':'));}}class
AbortException
extends
Exception{}class
...
__construct(DIContainer$context){$this->context=$context;}final
function
getContext(){return$this->context;}function
run(PresenterRequest$request){$this->request=$request;$httpRequest=$this->context->getByType('IHttpRequest');if(!$httpRequest->isAjax()&&($request->isMethod('get')||$request->isMethod('head'))){$refUrl=clone$httpRequest->getUrl();$url=$this->context->getService('router')->constructUrl($request,$refUrl->setPath($refUrl->getScriptPath()));if($url!==NULL&&!$httpRequest->getUrl()->isEqual($url)){return
new
RedirectResponse($url,IHttpResponse::S301_MOVED_PERMANENTLY);}}$params=$request->getParameters();if(!isset($params['callback'])){throw
new
BadRequestException("Parameter callback is missing.");}$params['presenter']=$this;$callback=new
Callback($params['callback']);$response=$callback->invokeArgs(PresenterComponentReflection::combineArgs($callback->toReflection(),$params));if(is_string($response)){$response=array($response,array());}if(is_array($response)){if($response[0]instanceof
SplFileInfo){$response=$this->createTemplate('FileTemplate')->setParameters($response[1])->setFile($response[0]);}else{$response=$this->createTemplate('Template')->setParameters($response[1])->setSource($response[0]);}}if($response
instanceof
ITemplate){return
new
TextResponse($response);}else{return$response;}}function
createTemplate($class=NULL,$latteFactory=NULL){$template=$class?new$class:new
FileTemplate;$template->setParameters($this->request->getParameters());$template->presenter=$this;$template->context=$context=$this->context;$url=$context->getByType('IHttpRequest')->getUrl();$template->baseUrl=rtrim($url->getBaseUrl(),'/');$template->basePath=rtrim($url->getBasePath(),'/');$template->registerHelperLoader('TemplateHelpers::loader');$template->setCacheStorage($context->getService('nette.templateCacheStorage'));$template->onPrepareFilters[]=create_function('$template','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('latteFactory'=>$latteFactory)).'-1], EXTR_REFS);
...
new
RedirectResponse($url,$code);}function
error($message=NULL,$code=IHttpResponse::S404_NOT_FOUND){throw
new
BadRequestException($message,$code);}function
...
__construct($baseDir,DIContainer$container){$this->baseDir=$baseDir;$this->container=$container;}function
...
instanceof
Presenter&&$presenter->invalidLinkMode===NULL){$presenter->invalidLinkMode=$this->container->parameters['debugMode']?Presenter::INVALID_LINK_WARNING:Presenter::INVALID_LINK_SILENT;}return$presenter;}function
getPresenterClass(&$name){if(isset($this->cache[$name])){list($class,$name)=$this->cache[$name];return$class;}if(!is_string($name)||!Strings::match($name,'#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*\z#')){throw
new
InvalidPresenterException("Presenter name must be alphanumeric string, '$name' is invalid.");}$class=$this->formatPresenterClass($name);if(!class_exists($class)){$file=$this->formatPresenterFile($name);if(is_file($file)&&is_readable($file)){LimitedScope::load($file,TRUE);}if(!class_exists($class)){throw
new
...
constructUrl(PresenterRequest$appRequest,Url$refUrl){return
NULL;}function
...
Closure||$metadata
instanceof
Callback){$metadata=array(self::PRESENTER_KEY=>'Nette:Micro','callback'=>$metadata);}$this->flags=$flags|self::$defaultFlags;$this->setMask($mask,$metadata);}function
...
constructUrl(PresenterRequest$appRequest,Url$refUrl){if($this->flags&self::ONE_WAY){return
...
NULL;}function
constructUrl(PresenterRequest$appRequest,Url$refUrl){if($this->cachedRoutes===NULL){$routes=array();$routes['*']=array();foreach($this
...
new
InvalidArgumentException("Argument must be array or string in format Presenter:action, '$defaults' given.");}$defaults=array(self::PRESENTER_KEY=>substr($defaults,0,$a),'action'=>$a===strlen($defaults)-1?Presenter::DEFAULT_ACTION:substr($defaults,$a+1));}if(isset($defaults[self::MODULE_KEY])){$this->module=$defaults[self::MODULE_KEY].':';unset($defaults[self::MODULE_KEY]);}$this->defaults=$defaults;$this->flags=$flags;}function
...
BadSignalException
extends
BadRequestException{protected$defaultCode=403;}abstract
class
Component
extends
Object
...
new
InvalidArgumentException("Component name must be integer or string, ".gettype($name)." given.");}elseif(!preg_match('#^[a-zA-Z0-9_]+\z#',$name)){throw
new
...
function
validateParent(IComponentContainer$parent){parent::validateParent($parent);$this->monitor('Presenter');}protected
function
...
new
BadRequestException("Invalid value for persistent parameter '$name' in '{$this->getName()}', expected ".($type==='NULL'?'scalar':$type).".");}$this->$name=&$params[$name];}else{$params[$name]=&$this->$name;}}$this->params=$params;}function
...
link($destination,$args=array()){try{$_args=func_get_args();return$this->getPresenter()->createRequest($this,$destination,is_array($args)?$args:array_slice($_args,1),'link');}catch(InvalidLinkException$e){return$this->getPresenter()->handleInvalidLink($e);}}function
lazyLink($destination,$args=array()){$_args=func_get_args();return
new
Link($this,$destination,is_array($args)?$args:array_slice($_args,1));}function
...
FileTemplate;$presenter=$this->getPresenter(FALSE);$template->onPrepareFilters[]=$this->templatePrepareFilters;$template->registerHelperLoader('TemplateHelpers::loader');$template->control=$template->_control=$this;$template->presenter=$template->_presenter=$presenter;if($presenter
instanceof
...
instanceof
FormContainer){if(array_key_exists($name,$values)){$control->setValues($values[$name],$erase);}elseif($erase){$control->setValues(array(),$erase);}}}return$this;}function
getValues($asArray=FALSE){$values=$asArray?array():new
ArrayHash;foreach($this->getComponents()as$name=>$control){if($control
instanceof
IFormControl&&!$control->isDisabled()&&!$control
instanceof
ISubmitterControl){$values[$name]=$control->getValue();}elseif($control
instanceof
FormContainer){$values[$name]=$control->getValues($asArray);}}return$values;}function
...
__construct($name=NULL){$this->element=Html::el('form');$this->element->action='';$this->element->method=self::POST;$this->element->id=$name===NULL?NULL:'frm-'.$name;$this->monitor(__CLASS__);if($name!==NULL){$tracker=new
...
addProtection($message=NULL,$timeout=NULL){$session=$this->getSession()->getSection('Nette.Forms.Form/CSRF');$key="key$timeout";if(isset($session->$key)){$token=$session->$key;}else{$session->$key=$token=Strings::random();}$session->setExpiration($timeout,$key);$this[self::PROTECTOR_ID]=new
...
function
receiveHttpData(){$httpRequest=$this->getHttpRequest();if(strcasecmp($this->getMethod(),$httpRequest->getMethod())){return;}if($httpRequest->isMethod('post')){$data=Arrays::mergeTree($httpRequest->getPost(),$httpRequest->getFiles());}else{$data=$httpRequest->getQuery();if(!$data){return;}}if($tracker=$this->getComponent(self::TRACKER_ID,FALSE)){if(!isset($data[self::TRACKER_ID])||$data[self::TRACKER_ID]!==$tracker->getValue()){return;}}return$data;}function
...
Arrays::mergeTree($request->getPost(),$request->getFiles());}else{return$request->getParameters();}}function
signalReceived($signal){if($signal==='submit'){if(!$this->getPresenter()->getRequest()->hasFlag(PresenterRequest::RESTORED)){$this->fireEvents();}}else{$class=get_class($this);throw
new
...
__construct(DIContainer$context=NULL){$this->context=$context;if($context&&$this->invalidLinkMode===NULL){$this->invalidLinkMode=$context->parameters['productionMode']?self::INVALID_LINK_SILENT:self::INVALID_LINK_WARNING;}}final
function
...
run(PresenterRequest$request){try{$this->request=$request;$this->payload=new
stdClass;$this->setParent($this->getParent(),$request->getPresenterName());$this->initGlobalParameters();$this->checkRequirements($this->getReflection());$this->startup();if(!$this->startupCheck){$class=$this->getReflection()->getMethod('startup')->getDeclaringClass()->getName();throw
new
...
instanceof
TextResponse&&$this->isControlInvalid()){$this->snippetMode=TRUE;$this->response->send($this->getHttpRequest(),$this->getHttpResponse());$this->sendPayload();}elseif(!$this->response&&$hasPayload){$this->sendPayload();}}catch(AbortException$e){}if($this->hasFlashSession()){$this->getFlashSession()->setExpiration($this->response
instanceof
RedirectResponse?'+ 30 seconds':'+ 3 seconds');}$this->onShutdown($this,$this->response);$this->shutdown($this->response);return$this->response;}}protected
function
...
new
ForbiddenRequestException;}}function
...
changeAction($action){if(is_string($action)&&Strings::match($action,'#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*\z#')){$this->action=$action;$this->view=$action;}else{$this->error('Action name is not alphanumeric string.');}}final
function
...
TextResponse($template));}function
...
ForwardResponse($destination));}$_args=func_get_args();$this->createRequest($this,$destination,is_array($args)?$args:array_slice($_args,1),'forward');$this->sendResponse(new
ForwardResponse($this->lastCreatedRequest));}function
...
RedirectResponse($url,IHttpResponse::S301_MOVED_PERMANENTLY));}}}function
...
new
InvalidLinkException("Missing presenter name in '$destination'.");}$presenter=substr($destination,1,$a-1);}else{$presenter=$this->getName();$b=strrpos($presenter,':');if($b===FALSE){$presenter=substr($destination,0,$a);}else{$presenter=substr($presenter,0,$b+1).substr($destination,0,$a);}}try{$presenterClass=$presenterFactory->getPresenterClass($presenter);}catch(InvalidPresenterException$e){throw
new
...
PresenterRequest($presenter,PresenterRequest::FORWARD,$args,array(),array());$this->lastCreatedRequestFlag=array('current'=>$current);if($mode==='forward'||$mode==='test'){return;}$url=$router->constructUrl($this->lastCreatedRequest,$refUrl);if($url===NULL){unset($args[self::ACTION_KEY]);$params=urldecode(http_build_query($args,NULL,', '));throw
new
...
storeRequest($expiration='+ 10 minutes'){$session=$this->getSession('Nette.Application/requests');do{$key=Strings::random(5);}while(isset($session[$key]));$session[$key]=array($this->getUser()->getId(),$this->request);$session->setExpiration($expiration,$key);return$key;}function
restoreRequest($key){$session=$this->getSession('Nette.Application/requests');if(!isset($session[$key])||($session[$key][0]!==NULL&&$session[$key][0]!==$this->getUser()->getId())){return;}$request=clone$session[$key][1];unset($session[$key]);$request->setFlag(PresenterRequest::RESTORED,TRUE);$params=$request->getParameters();$params[self::FLASH_KEY]=$this->getParameter(self::FLASH_KEY);$request->setParameters($params);$this->sendResponse(new
ForwardResponse($request));}static
function
...
PresenterComponentReflection($forClass):NULL);if($sinces===NULL){$sinces=array();foreach($this->getReflection()->getPersistentParams()as$name=>$meta){$sinces[$name]=$meta['since'];}}$components=$this->getReflection()->getPersistentComponents();$iterator=$this->getComponents(TRUE,'IStatePersistent');foreach($iterator
...
getFlashSession(){if(empty($this->params[self::FLASH_KEY])){$this->params[self::FLASH_KEY]=Strings::random(4);}return$this->getSession('Nette.Application.Flash/'.$this->params[self::FLASH_KEY]);}final
function
injectPrimary(DIContainer$context){$this->context=$context;}final
function
...
function
getHttpContext(){return$this->context->getByType('HttpContext');}function
getApplication(){return$this->context->getByType('Application');}function
getSession($namespace=NULL){$handler=$this->context->getByType('Session');return$namespace===NULL?$handler:$handler->getSection($namespace);}function
getUser(){return$this->context->getByType('User');}}class
ClassReflection
extends
...
Callback($callback);$l['']=NULL;return$this;}function
...
Callback($fce);self::$extMethods[$pair[1]]['']=NULL;}}if($name===NULL){return
...
getPersistentComponents($class=NULL){$class=$class===NULL?$this->getName():$class;$components=&self::$pcCache[$class];if($components!==NULL){return$components;}$components=array();if(is_subclass_of($class,'Presenter')){foreach(call_user_func(array($class,'getPersistentComponents'),$class)as$name=>$meta){if(is_string($meta)){$name=$meta;}$components[$name]=array('since'=>$class);}$components=$this->getPersistentComponents(get_parent_class($class))+$components;}return$components;}function
hasCallableMethod($method){$class=$this->getName();$cache=&self::$mcCache[strtolower($class.':'.$method)];if($cache===NULL)try{$cache=FALSE;$rm=MethodReflection::from($class,$method);$cache=$this->isInstantiable()&&$rm->isPublic()&&!$rm->isAbstract()&&!$rm->isStatic();}catch(ReflectionException$e){}return$cache;}static
function
...
call($function){$key=func_get_args();return$this->load($key,create_function('','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('function'=>$function,'key'=>$key)).'-1], EXTR_REFS);
			return Callback::create($function)->invokeArgs(array_slice($key, 1));
		'));}function
...
create_function('','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('cache'=>$cache,'function'=>$function,'dependencies'=>$dependencies)).'-1], EXTR_REFS);
			...
			$data = $cache->load($key);
			if ($data === NULL) {
				$data = $cache->save($key, Callback::create($function)->invokeArgs($key[1]), $dependencies);
			}
			return $data;
		');}function
start($key){$data=$this->load($key);if($data===NULL){return
new
CachingHelper($this,$key);}echo$data;}protected
function
...
CachingHelper
extends
Object{public$dependencies;private$cache;private$key;function
...
clean(array$conditions){if(!empty($conditions[Cache::ALL])){$this->data=array();}}}class
PhpFileStorage
extends
...
parent::getCacheFile(substr_replace($key,trim(strtr($this->hint,'\\/@','.._'),'.').'-',strpos($key,Cache::NAMESPACE_SEPARATOR)+1,0)).'.php';}}class
ArrayHash
extends
stdClass
implements
...
instanceof
Closure){$cb=array($cb,'__invoke');}if(is_string($this->cb)&&$a=strrpos($this->cb,'\\')){$this->cb=substr($this->cb,$a+1);}elseif(is_array($this->cb)&&is_string($this->cb[0])&&$a=strrpos($this->cb[0],'\\')){$this->cb[0]=substr($this->cb[0],$a+1);}if(!is_callable($cb,TRUE)){throw
new
...
new
MethodReflection($this->cb);}elseif(is_array($this->cb)){return
new
MethodReflection($this->cb[0],$this->cb[1]);}elseif(is_object($this->cb)&&!$this->cb
instanceof
Closure){return
new
MethodReflection($this->cb,'__invoke');}else{return
new
FunctionReflection($this->cb);}}function
isStatic(){return
...
PHP_SAPI==='cli';}static
function
isProduction(){if(self::$productionMode===NULL){self::$productionMode=!Configurator::detectDebugMode();}return
...
function
setContext(DIContainer$context){if(self::$createdAt){throw
new
InvalidStateException('Configurator & SystemContainer has already been created automatically by Environment at '.self::$createdAt);}self::$context=$context;}static
function
...
self::getContext()->getByType('HttpContext');}static
function
getHttpResponse(){return
self::getContext()->getByType('IHttpResponse');}static
function
getApplication(){return
self::getContext()->getByType('Application');}static
function
getUser(){return
self::getContext()->getByType('User');}static
function
getRobotLoader(){return
self::getContext()->getByType('RobotLoader');}static
function
getCache($namespace=''){return
new
Cache(self::getService('cacheStorage'),$namespace);}static
function
...
function
getFormatFromString($s){$types=array('image/jpeg'=>self::JPEG,'image/gif'=>self::GIF,'image/png'=>self::PNG);$type=MimeTypeDetector::fromString($s);return
...
new
UnexpectedValueException("Property $class::$$name must be array or NULL, ".gettype($_this->$name)." given.");}}elseif($cb=ClassReflection::from($_this)->getExtensionMethod($name)){array_unshift($args,$_this);return$cb->invokeArgs($args);}else{throw
new
...
new
InvalidStateException("Invalid key '$key' in section [$secName] in file '$file'.");}}$cursor=$val;}$secData=$tmp;}$parts=explode(self::INHERITING_SEPARATOR,$secName);if(count($parts)>1){$secName=trim($parts[0]);$secData[ConfigHelpers::EXTENDS_KEY]=trim($parts[1]);}}$cursor=&$data;foreach(explode(self::KEY_SEPARATOR,$secName)as$part){if(!isset($cursor[$part])||is_array($cursor[$part])){$cursor=&$cursor[$part];}else{throw
new
InvalidStateException("Invalid section [$secName] in file '$file'.");}}if(is_array($secData)&&is_array($cursor)){$secData=ConfigHelpers::merge($secData,$cursor);}$cursor=$secData;}return$data;}function
dump(array$data){$output=array();foreach($data
as$name=>$secData){if(!is_array($secData)){$output=array();self::build($data,$output,'');break;}if($parent=ConfigHelpers::takeParent($secData)){$output[]="[$name ".self::INHERITING_SEPARATOR." $parent]";}else{$output[]="[$name]";}self::build($secData,$output,'');$output[]='';}return"; generated by Nette\n\n".implode(PHP_EOL,$output);}private
static
function
...
new
InvalidStateException("Replacing operator is available only for arrays, item '$key' is not array.");}$key=substr($key,0,-1);$val[ConfigHelpers::EXTENDS_KEY]=ConfigHelpers::OVERWRITE;}elseif(preg_match('#^(\S+)\s+'.self::INHERITING_SEPARATOR.'\s+(\S+)\z#',$key,$matches)){if(!is_array($val)&&$val!==NULL){throw
new
InvalidStateException("Inheritance operator is available only for arrays, item '$key' is not array.");}list(,$key,$val[ConfigHelpers::EXTENDS_KEY])=$matches;if(isset($res[$key])){throw
new
InvalidStateException("Duplicated key '$key'.");}}if(is_array($val)){$val=$this->process($val);}elseif($val
instanceof
NeonEntity){$val=(object)array('value'=>$val->value,'attributes'=>$this->process($val->attributes));}$res[$key]=$val;}return$res;}function
dump(array$data){$tmp=array();foreach($data
as$name=>$secData){if($parent=ConfigHelpers::takeParent($secData)){$name.=' '.self::INHERITING_SEPARATOR.' '.$parent;}$tmp[$name]=$secData;}return"# generated by Nette\n\n".Neon::encode($tmp,Neon::BLOCK);}}class
ConfigPhpAdapter
extends
Object
implements
IConfigAdapter{function
load($file){return
LimitedScope::load($file);}function
dump(array$data){return"<?php // generated by Nette \nreturn ".PhpHelpers::dump($data).';';}}class
ConfigCompiler
extends
Object{private$extensions=array();private$container;private$config;private
static$reserved=array('services'=>1,'factories'=>1,'parameters'=>1);function
addExtension($name,ConfigCompilerExtension$extension){if(isset(self::$reserved[$name])){throw
new
...
DIContainerBuilder;$this->processParameters();$this->processExtensions();$this->processServices();return$this->generateCode($className,$parentName);}function
...
as$name=>$extension){$this->container->addDefinition($name)->setClass('DINestedAccessor',array('@container',$name))->setAutowired(FALSE);if(isset($this->config[$name])){$this->parseServices($this->container,$this->config[$name],$name);}}foreach($this->container->getDefinitions()as$name=>$def){$factory=$name.'Factory';if(!$def->shared&&!$def->internal&&!$this->container->hasDefinition($factory)){$this->container->addDefinition($factory)->setClass('Callback',array('@container',DIContainer::getMethodName($name,FALSE)))->setAutowired(FALSE)->tags=$def->tags;}}}function
generateCode($className,$parentName){foreach($this->extensions
as$extension){$extension->beforeCompile();$this->container->addDependency(ClassReflection::from($extension)->getFileName());}$classes[]=$class=$this->container->generateClass($parentName);$class->setName($className)->addMethod('initialize');foreach($this->extensions
as$extension){$extension->afterCompile($class);}$defs=$this->container->getDefinitions();ksort($defs);$list=array_keys($defs);foreach(array_reverse($defs,TRUE)as$name=>$def){if($def->class==='DINestedAccessor'&&($found=preg_grep('#^'.$name.'\.#i',$list))){$list=array_diff($list,$found);$def->class=$className.'_'.preg_replace('#\W+#','_',$name);$class->documents=preg_replace("#\\S+(?= \\$$name\\z)#",$def->class,$class->documents);$classes[]=$accessor=new
PhpClassType($def->class);foreach($found
...
implode("\n\n\n",$classes);}static
function
parseServices(DIContainerBuilder$container,array$config,$namespace=NULL){$services=isset($config['services'])?$config['services']:array();$factories=isset($config['factories'])?$config['factories']:array();$all=array_merge($services,$factories);uasort($all,create_function('$a, $b','
...
new
ServiceCreationException("It is not allowed to use services and factories with the same name: '$origName'.");}else{$name=($namespace?$namespace.'.':'').strtr($origName,'\\','_');}if(($parent=ConfigHelpers::takeParent($def))&&$parent!==$name){$container->removeDefinition($name);$definition=$container->addDefinition($name);if($parent!==ConfigHelpers::OVERWRITE){foreach($container->getDefinition($parent)as$k=>$v){$definition->$k=unserialize(serialize($v));}}}elseif($container->hasDefinition($name)){$definition=$container->getDefinition($name);if($definition->shared!==$shared){throw
new
ServiceCreationException("It is not allowed to use service and factory with the same name '$name'.");}}else{$definition=$container->addDefinition($name);}try{self::parseService($definition,$def,$shared);}catch(Exception$e){throw
new
ServiceCreationException("Service '$name': ".$e->getMessage(),NULL,$e);}}}static
function
parseService(DIServiceDefinition$definition,$config,$shared=TRUE){if($config===NULL){return;}elseif(!is_array($config)){$config=array('class'=>NULL,'factory'=>$config);}$known=$shared?array('class','factory','arguments','setup','autowired','run','tags'):array('class','factory','arguments','setup','autowired','tags','internal','parameters');if($error=array_diff(array_keys($config),$known)){throw
new
InvalidStateException("Unknown key '".implode("', '",$error)."' in definition of service.");}$arguments=array();if(array_key_exists('arguments',$config)){Validators::assertField($config,'arguments','array');$arguments=self::filterArguments($config['arguments']);$definition->setArguments($arguments);}if(array_key_exists('class',$config)||array_key_exists('factory',$config)){$definition->class=NULL;$definition->factory=NULL;}if(array_key_exists('class',$config)){Validators::assertField($config,'class','string|stdClass|null');if($config['class']instanceof
stdClass){$definition->setClass($config['class']->value,self::filterArguments($config['class']->attributes));}else{$definition->setClass($config['class'],$arguments);}}if(array_key_exists('factory',$config)){Validators::assertField($config,'factory','callable|stdClass|null');if($config['factory']instanceof
stdClass){$definition->setFactory($config['factory']->value,self::filterArguments($config['factory']->attributes));}else{$definition->setFactory($config['factory'],$arguments);}}if(isset($config['setup'])){if(ConfigHelpers::takeParent($config['setup'])){$definition->setup=array();}Validators::assertField($config,'setup','list');foreach($config['setup']as$id=>$setup){Validators::assert($setup,'callable|stdClass',"setup item #$id");if($setup
instanceof
stdClass){Validators::assert($setup->value,'callable',"setup item #$id");$definition->addSetup($setup->value,self::filterArguments($setup->attributes));}else{$definition->addSetup($setup);}}}$definition->setShared($shared);if(isset($config['parameters'])){Validators::assertField($config,'parameters','array');$definition->setParameters($config['parameters']);}if(isset($config['autowired'])){Validators::assertField($config,'autowired','bool');$definition->setAutowired($config['autowired']);}if(isset($config['internal'])){Validators::assertField($config,'internal','bool');$definition->setInternal($config['internal']);}if(isset($config['run'])){$config['tags']['run']=(bool)$config['run'];}if(isset($config['tags'])){Validators::assertField($config,'tags','array');if(ConfigHelpers::takeParent($config['tags'])){$definition->tags=array();}foreach($config['tags']as$tag=>$attrs){if(is_int($tag)&&is_string($attrs)){$definition->addTag($attrs);}else{$definition->addTag($tag,$attrs);}}}}static
function
filterArguments(array$args){foreach($args
as$k=>$v){if($v==='...'){unset($args[$k]);}elseif($v
instanceof
stdClass&&isset($v->value,$v->attributes)){$args[$k]=new
DIStatement($v->value,self::filterArguments($v->attributes));}}return$args;}}abstract
class
ConfigCompilerExtension
extends
Object{protected$compiler;protected$name;function
setCompiler(ConfigCompiler$compiler,$name){$this->compiler=$compiler;$this->name=$name;return$this;}function
getConfig(array$defaults=NULL,$expand=TRUE){$config=$this->compiler->getConfig();$config=isset($config[$this->name])?$config[$this->name]:array();unset($config['services'],$config['factories']);$config=ConfigHelpers::merge($config,$defaults);return$expand?$this->compiler->getContainerBuilder()->expand($config):$config;}function
getContainerBuilder(){return$this->compiler->getContainerBuilder();}function
loadFromFile($file){$loader=new
ConfigLoader;$res=$loader->load($file);$container=$this->compiler->getContainerBuilder();foreach($loader->getDependencies()as$file){$container->addDependency($file);}return$res;}function
prefix($id){return
substr_replace($id,$this->name.'.',substr($id,0,1)==='@'?1:0,0);}function
loadConfiguration(){}function
beforeCompile(){}function
afterCompile(PhpClassType$class){}}class
Configurator
extends
Object{const
...
addParameters(array$params){$this->parameters=ConfigHelpers::merge($params,$this->parameters);return$this;}protected
function
...
array('appDir'=>isset($trace[1]['file'])?dirname($trace[1]['file']):NULL,'wwwDir'=>isset($_SERVER['SCRIPT_FILENAME'])?dirname($_SERVER['SCRIPT_FILENAME']):NULL,'debugMode'=>$debugMode,'productionMode'=>!$debugMode,'environment'=>$debugMode?'development':'production','consoleMode'=>PHP_SAPI==='cli','container'=>array('class'=>'SystemContainer','parent'=>'DIContainer'));}function
enableDebugger($logDirectory=NULL,$email=NULL){Debugger::$strictMode=TRUE;Debugger::enable($this->parameters['productionMode'],$logDirectory,$email);}function
...
RobotLoader;$loader->setCacheStorage(new
FileStorage($cacheDir));$loader->autoRebuild=!$this->parameters['productionMode'];return$loader;}function
...
Cache(new
PhpFileStorage($cacheDir),'Nette.Configurator');$cacheKey=array($this->parameters,$this->files);$cached=$cache->load($cacheKey);if(!$cached){$code=$this->buildContainer($dependencies);$cache->save($cacheKey,$code,array(Cache::FILES=>$dependencies));$cached=$cache->load($cacheKey);}LimitedScope::load($cached['file'],TRUE);}elseif($this->files){throw
new
InvalidStateException("Set path to temporary directory using setTempDirectory().");}else{LimitedScope::evaluate($this->buildContainer());}$container=new$this->parameters['container']['class'];$container->initialize();Environment::setContext($container);return$container;}protected
function
buildContainer(&$dependencies=NULL){$loader=$this->createLoader();$config=array();$code="<?php\n";foreach($this->files
as$tmp){list($file,$section)=$tmp;$config=ConfigHelpers::merge($loader->load($file,$section),$config);$code.="// source: $file $section\n";}$code.="\n";$this->checkCompatibility($config);if(!isset($config['parameters'])){$config['parameters']=array();}$config['parameters']=ConfigHelpers::merge($config['parameters'],$this->parameters);$compiler=$this->createCompiler();$this->onCompile($this,$compiler);$code.=$compiler->compile($config,$this->parameters['container']['class'],$config['parameters']['container']['parent']);$dependencies=array_merge($loader->getDependencies(),$this->isDebugMode()?$compiler->getContainerBuilder()->getDependencies():array());return$code;}protected
function
...
ConfigCompiler;$compiler->addExtension('php',new
PhpExtension)->addExtension('constants',new
ConstantsExtension)->addExtension('nette',new
NetteExtension);return$compiler;}protected
function
createLoader(){return
new
...
ConstantsExtension
extends
ConfigCompilerExtension{function
afterCompile(PhpClassType$class){foreach($this->getConfig()as$name=>$value){$class->methods['initialize']->addBody('define(?, ?);',array($name,$value));}}}class
NetteExtension
extends
ConfigCompilerExtension{public$defaults=array('xhtml'=>TRUE,'session'=>array('iAmUsingBadHost'=>NULL,'autoStart'=>'smart','expiration'=>NULL),'application'=>array('debugger'=>TRUE,'errorPresenter'=>NULL,'catchExceptions'=>'%productionMode%'),'routing'=>array('debugger'=>TRUE,'routes'=>array()),'security'=>array('debugger'=>TRUE,'frames'=>'SAMEORIGIN','users'=>array(),'roles'=>array(),'resources'=>array()),'mailer'=>array('smtp'=>FALSE),'database'=>array(),'forms'=>array('messages'=>array()),'container'=>array('debugger'=>FALSE),'debugger'=>array('email'=>NULL,'editor'=>NULL,'browser'=>NULL,'strictMode'=>NULL,'bar'=>array(),'blueScreen'=>array()));public$databaseDefaults=array('dsn'=>NULL,'user'=>NULL,'password'=>NULL,'options'=>NULL,'debugger'=>TRUE,'explain'=>TRUE,'reflection'=>'DiscoveredReflection');function
loadConfiguration(){$container=$this->getContainerBuilder();$config=$this->getConfig($this->defaults);$container->addDefinition($this->prefix('cacheJournal'))->setClass('FileJournal',array('%tempDir%'));$container->addDefinition('cacheStorage')->setClass('FileStorage',array('%tempDir%/cache'));$container->addDefinition($this->prefix('templateCacheStorage'))->setClass('PhpFileStorage',array('%tempDir%/cache'))->setAutowired(FALSE);$container->addDefinition($this->prefix('cache'))->setClass('Cache',array(1=>'%namespace%'))->setParameters(array('namespace'=>NULL));$container->addDefinition($this->prefix('httpRequestFactory'))->setClass('HttpRequestFactory')->addSetup('setEncoding',array('UTF-8'))->setInternal(TRUE);$container->addDefinition('httpRequest')->setClass('HttpRequest')->setFactory('@\HttpRequestFactory::createHttpRequest');$container->addDefinition('httpResponse')->setClass('HttpResponse');$container->addDefinition($this->prefix('httpContext'))->setClass('HttpContext');$session=$container->addDefinition('session')->setClass('Session');if(isset($config['session']['expiration'])){$session->addSetup('setExpiration',array($config['session']['expiration']));}if(isset($config['session']['iAmUsingBadHost'])){$session->addSetup('Framework::$iAmUsingBadHost = ?;',array((bool)$config['session']['iAmUsingBadHost']));}unset($config['session']['expiration'],$config['session']['autoStart'],$config['session']['iAmUsingBadHost']);if(!empty($config['session'])){$session->addSetup('setOptions',array($config['session']));}$container->addDefinition($this->prefix('userStorage'))->setClass('UserStorage');$user=$container->addDefinition('user')->setClass('User');if(!$container->parameters['productionMode']&&$config['security']['debugger']){$user->addSetup('Debugger::$bar->addPanel(?)',array(new
DIStatement('UserPanel')));}if($config['security']['users']){$container->addDefinition($this->prefix('authenticator'))->setClass('SimpleAuthenticator',array($config['security']['users']));}if($config['security']['roles']||$config['security']['resources']){$authorizator=$container->addDefinition($this->prefix('authorizator'))->setClass('Permission');foreach($config['security']['roles']as$role=>$parents){$authorizator->addSetup('addRole',array($role,$parents));}foreach($config['security']['resources']as$resource=>$parents){$authorizator->addSetup('addResource',array($resource,$parents));}}$application=$container->addDefinition('application')->setClass('Application')->addSetup('$catchExceptions',$config['application']['catchExceptions'])->addSetup('$errorPresenter',$config['application']['errorPresenter']);if($config['application']['debugger']){$application->addSetup('RoutingDebugger::initializePanel');}$container->addDefinition($this->prefix('presenterFactory'))->setClass('PresenterFactory',array(isset($container->parameters['appDir'])?$container->parameters['appDir']:NULL));$router=$container->addDefinition('router')->setClass('RouteList');foreach($config['routing']['routes']as$mask=>$action){$router->addSetup('$service[] = new Route(?, ?);',array($mask,$action));}if(!$container->parameters['productionMode']&&$config['routing']['debugger']){$application->addSetup('Debugger::$bar->addPanel(?)',array(new
DIStatement('RoutingDebugger')));}if(empty($config['mailer']['smtp'])){$container->addDefinition($this->prefix('mailer'))->setClass('SendmailMailer');}else{$container->addDefinition($this->prefix('mailer'))->setClass('SmtpMailer',array($config['mailer']));}$container->addDefinition($this->prefix('mail'))->setClass('Mail')->addSetup('setMailer')->setShared(FALSE);$container->addDefinition($this->prefix('basicForm'))->setClass('Form')->setShared(FALSE);$latte=$container->addDefinition($this->prefix('latte'))->setClass('LatteFilter')->setShared(FALSE);if(empty($config['xhtml'])){$latte->addSetup('$service->getCompiler()->defaultContentType = ?',LatteCompiler::CONTENT_HTML);}$container->addDefinition($this->prefix('template'))->setClass('FileTemplate')->addSetup('registerFilter',array($latte))->addSetup('registerHelperLoader',array('TemplateHelpers::loader'))->setShared(FALSE);$container->addDefinition($this->prefix('database'))->setClass('DINestedAccessor',array('@container',$this->prefix('database')));if(isset($config['database']['dsn'])){$config['database']=array('default'=>$config['database']);}$autowired=TRUE;foreach((array)$config['database']as$name=>$info){if(!is_array($info)){continue;}$info+=$this->databaseDefaults+array('autowired'=>$autowired);$autowired=FALSE;foreach((array)$info['options']as$key=>$value){if(preg_match('#^PDO::\w+\z#',$key)){unset($info['options'][$key]);$info['options'][constant($key)]=$value;}}$connection=$container->addDefinition($this->prefix("database.$name"))->setClass('Connection',array($info['dsn'],$info['user'],$info['password'],$info['options']))->setAutowired($info['autowired'])->addSetup('setCacheStorage')->addSetup('Debugger::$blueScreen->addPanel(?)',array('DatabasePanel::renderException'));if($info['reflection']){$connection->addSetup('setDatabaseReflection',is_string($info['reflection'])?array(new
DIStatement(preg_match('#^[a-z]+\z#',$info['reflection'])?'Nette\Database\Reflection\\'.ucfirst($info['reflection']).'Reflection':$info['reflection'])):ConfigCompiler::filterArguments(array($info['reflection'])));}if(!$container->parameters['productionMode']&&$info['debugger']){$panel=$container->addDefinition($this->prefix("database.{$name}ConnectionPanel"))->setClass('DatabasePanel')->setAutowired(FALSE)->addSetup('$explain',!empty($info['explain']))->addSetup('$name',$name)->addSetup('Debugger::$bar->addPanel(?)',array('@self'));$connection->addSetup('$service->onQuery[] = ?',array(array($panel,'logQuery')));}}}function
afterCompile(PhpClassType$class){$initialize=$class->methods['initialize'];$container=$this->getContainerBuilder();$config=$this->getConfig($this->defaults);foreach(array('email','editor','browser','strictMode','maxLen','maxDepth')as$key){if(isset($config['debugger'][$key])){$initialize->addBody('Debugger::$? = ?;',array($key,$config['debugger'][$key]));}}if(!$container->parameters['productionMode']){if($config['container']['debugger']){$config['debugger']['bar'][]='ContainerPanel';}foreach((array)$config['debugger']['bar']as$item){$initialize->addBody($container->formatPhp('Debugger::$bar->addPanel(?);',ConfigCompiler::filterArguments(array(is_string($item)?new
DIStatement($item):$item))));}foreach((array)$config['debugger']['blueScreen']as$item){$initialize->addBody($container->formatPhp('Debugger::$blueScreen->addPanel(?);',ConfigCompiler::filterArguments(array($item))));}}if(!empty($container->parameters['tempDir'])){$initialize->addBody($this->checkTempDir($container->expand('%tempDir%/cache')));}foreach((array)$config['forms']['messages']as$name=>$text){$initialize->addBody('Rules::$defaultMessages[Form::?] = ?;',array($name,$text));}if($config['session']['autoStart']==='smart'){$initialize->addBody('$this->getService("session")->exists() && $this->getService("session")->start();');}elseif($config['session']['autoStart']){$initialize->addBody('$this->getService("session")->start();');}if(empty($config['xhtml'])){$initialize->addBody('Html::$xhtml = ?;',array((bool)$config['xhtml']));}if(isset($config['security']['frames'])&&$config['security']['frames']!==TRUE){$frames=$config['security']['frames'];if($frames===FALSE){$frames='DENY';}elseif(preg_match('#^https?:#',$frames)){$frames="ALLOW-FROM $frames";}$initialize->addBody('header(?);',array("X-Frame-Options: $frames"));}foreach($container->findByTag('run')as$name=>$on){if($on){$initialize->addBody('$this->getService(?);',array($name));}}}private
function
checkTempDir($dir){$uniq=uniqid('_',TRUE);if(!@mkdir("$dir/$uniq",0777)){throw
new
InvalidStateException("Unable to write to directory '$dir'. Make this directory writable.");}$useDirs=@file_put_contents("$dir/$uniq/_",'')!==FALSE;@unlink("$dir/$uniq/_");@rmdir("$dir/$uniq");return'FileStorage::$useDirectories = '.($useDirs?'TRUE':'FALSE').";\n";}}class
PhpExtension
extends
ConfigCompilerExtension{function
afterCompile(PhpClassType$class){$initialize=$class->methods['initialize'];foreach($this->getConfig()as$name=>$value){if(!is_scalar($value)){throw
new
...
extends
Object{const
INCLUDES_KEY='includes';private$adapters=array('php'=>'ConfigPhpAdapter','ini'=>'ConfigIniAdapter','neon'=>'ConfigNeonAdapter');private$dependencies=array();function
...
new
InvalidStateException("Section 'includes' must be placed under some top section in file '$file'.");}$data=$this->getSection($data,$section,$file);}$merged=array();if(isset($data[self::INCLUDES_KEY])){Validators::assert($data[self::INCLUDES_KEY],'list',"section 'includes' in file '$file'");foreach($data[self::INCLUDES_KEY]as$include){$merged=ConfigHelpers::merge($this->load(dirname($file).'/'.$include),$merged);}}unset($data[self::INCLUDES_KEY]);return
ConfigHelpers::merge($data,$merged);}function
...
function
getSection(array$data,$key,$file){Validators::assertField($data,$key,'array|null',"section '%' in file '$file'");$item=$data[$key];if($parent=ConfigHelpers::takeParent($item)){$item=ConfigHelpers::merge($item,$this->getSection($data,$parent,$file));}return$item;}}if(class_exists('PDO')){class
Connection
extends
PDO{private$dsn;private$driver;private$preprocessor;private$databaseReflection;private$cache;public$onQuery;function
__construct($dsn,$username=NULL,$password=NULL,array$options=NULL,$driverClass=NULL){parent::__construct($this->dsn=$dsn,$username,$password,$options);$this->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);$this->setAttribute(PDO::ATTR_STATEMENT_CLASS,array('Statement',array($this)));$driverClass=($tmp=$driverClass)?$tmp:''.ucfirst(str_replace('sql','Sql',$this->getAttribute(PDO::ATTR_DRIVER_NAME))).'Driver';$this->driver=new$driverClass($this,(array)$options);$this->preprocessor=new
...
ConventionalReflection);}return$this->databaseReflection;}function
setCacheStorage(ICacheStorage$storage=NULL){$this->cache=$storage?new
Cache($storage,'Nette.Database.'.md5($this->dsn)):NULL;return$this;}function
...
logQuery(Statement$result,array$params=NULL){if($this->disabled){return;}$source=NULL;foreach(PHP_VERSION_ID<50205?debug_backtrace():debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],NETTE_DIR.DIRECTORY_SEPARATOR)!==0){if(isset($row['function'])&&strpos($row['function'],'call_user_func')===0)continue;if(isset($row['class'])&&is_subclass_of($row['class'],'\\Nette\\Database\\Connection'))continue;$source=array($row['file'],(int)$row['line']);break;}}$this->totalTime+=$result->getTime();$this->queries[]=array($result->queryString,$params,$result->getTime(),$result->rowCount(),$result->getConnection(),$source);}static
function
renderException($e){if(!$e
instanceof
PDOException){return;}if(isset($e->queryString)){$sql=$e->queryString;}elseif($item=DebugHelpers::findTrace($e->getTrace(),'PDO::prepare')){$sql=$item['args'][0];}return
isset($sql)?array('tab'=>'SQL','panel'=>DatabaseHelpers::dumpSql($sql)):NULL;}function
...
as$col){$s.='<td>'.htmlSpecialChars($col).'</td>';}$s.="</tr>";}$s.="</table>";}if($source){$s.=DebugHelpers::editorLink($source[0],$source[1])->class('nette-DbConnectionPanel-source');}$s.='</td><td>';foreach($params
...
__construct(Connection$connection,array$options){$this->connection=$connection;$charset=isset($options['charset'])?$options['charset']:'utf8';if($charset){$connection->query("SET NAMES '$charset'");}if(isset($options['sqlmode'])){$connection->query("SET sql_mode='$options[sqlmode]'");}$connection->query("SET time_zone='".date('P')."'");}function
...
setConnection(Connection$connection){}protected
function
...
setConnection(Connection$connection){$this->connection=$connection;if($this->cacheStorage){$this->cache=new
Cache($this->cacheStorage,'Nette.Database.'.md5($connection->getDsn()));$this->structure=$this->loadedStructure=($tmp=$this->cache->load('structure'))?$tmp:$this->structure;}}function
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$this->arrayMode='assoc';$sql=Strings::replace($sql,'~\'.*?\'|".*?"|\?|\b(?:INSERT|REPLACE|UPDATE)\b~si',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
instanceof
TableRow){return$value->getPrimary();}elseif(is_array($value)||$value
instanceof
...
instanceof
SqlLiteral){return$value->__toString();}else{$this->remaining[]=$value;return'?';}}}if(class_exists('PDO')){class
Statement
extends
...
DateTime53($value);}}return$this->connection->getSupplementalDriver()->normalizeRow($row,$this);}private
function
detectColumnTypes(){if($this->types===NULL){$this->types=array();if($this->connection->getSupplementalDriver()->isSupported(ISupplementalDriver::SUPPORT_COLUMNS_META)){$count=$this->columnCount();for($col=0;$col<$count;$col++){$meta=$this->getColumnMeta($col);if(isset($meta['native_type'])){$this->types[$meta['name']]=DatabaseHelpers::detectType($meta['native_type']);}}}}return$this->types;}function
getTime(){return$this->time;}function
dump(){DatabaseHelpers::dumpResult($this);}function
getReflection(){return
new
ClassReflection($this);}function
__call($name,$args){return
...
setTable(TableSelection$table){$this->table=$table;}function
...
instanceof
TableRow?$value->getPrimary():$value;$referenced=$this->table->getReferencedTable($table,$column,!empty($this->modified[$column]));$referenced=isset($referenced[$value])?$referenced[$value]:NULL;if(!empty($this->modified[$column])){$this->modified[$column]=0;}return$referenced;}return
FALSE;}}class
TableSelection
extends
Object
...
__construct($table,Connection$connection){$this->name=$table;$this->connection=$connection;$reflection=$connection->getDatabaseReflection();$this->primary=$reflection->getPrimary($table);$this->sqlBuilder=new
...
wherePrimary($key){if(is_array($this->primary)&&Validators::isList($key)){foreach($this->primary
...
new
TableRow($row,$this);}protected
function
createSelectionInstance($table=NULL){return
new
TableSelection(($tmp=$table)?$tmp:$this->name,$this->connection);}protected
function
createGroupedSelectionInstance($table,$column){return
new
GroupedTableSelection($this,$table,$column);}protected
function
...
TableRow?$row[$column]->getPrimary():$row[$column];$keys[$key]=TRUE;}if($referenced!==NULL){$a=array_keys($keys);$b=array_keys($referenced->rows);sort($a);sort($b);if($a===$b){return$referenced;}}if($keys){$referenced=$this->createSelectionInstance($table);$referenced->where($referenced->getPrimary(),array_keys($keys));}else{$referenced=array();}}return$referenced;}function
...
extends
TableSelection{protected$refTable;protected$column;protected$active;function
__construct(TableSelection$refTable,$table,$column){parent::__construct($table,$refTable->connection);$this->refTable=$refTable;$this->column=$column;}function
...
instanceof
GroupedTableSelection){$refPath.=$refObj->name.'.';$refObj=$refObj->refTable;}return$refObj;}function
insert($data){if($data
instanceof
Traversable&&!$data
instanceof
TableSelection){$data=iterator_to_array($data);}if(Validators::isList($data)){foreach(array_keys($data)as$key){$data[$key][$this->column]=$this->active;}}else{$data[$this->column]=$this->active;}return
...
instanceof
TableSelection){$clone=clone$arg;if(!$clone->getSqlBuilder()->select){try{$clone->select($clone->getPrimary());}catch(LogicException$e){throw
new
...
Closure&&!$service
instanceof
Callback){$this->registry[$name]=$service;$this->meta[$name]=$meta;return$this;}elseif(!is_string($service)||strpos($service,':')!==FALSE||$service[0]==="\0"){$service=new
Callback($service);}$this->factories[$name]=array($service);$this->registry[$name]=&$this->factories[$name][1];$this->meta[$name]=$meta;return$this;}function
...
new
InvalidStateException("Unable to create service '$name', factory '$factory' is not callable.");}else{$this->creating[$name]=TRUE;try{$service=$factory->invoke($this);}catch(Exception$e){}}}elseif(method_exists($this,$factory=DIContainer::getMethodName($name))&&$this->getReflection()->getMethod($factory)->getName()===$factory){$this->creating[$name]=TRUE;try{$service=$this->$factory();}catch(Exception$e){}}else{throw
new
...
isset($this->registry[$name])||isset($this->factories[$name])||method_exists($this,$method=DIContainer::getMethodName($name))&&$this->getReflection()->getMethod($method)->getName()===$method;}function
...
createInstance($class,array$args=array()){$rc=ClassReflection::from($class);if(!$rc->isInstantiable()){throw
new
ServiceCreationException("Class $class is not instantiable.");}elseif($constructor=$rc->getConstructor()){return$rc->newInstanceArgs(DIHelpers::autowireArguments($constructor,$args,$this));}elseif($args){throw
new
...
Callback($function);return$callback->invokeArgs(DIHelpers::autowireArguments($callback->toReflection(),$args,$this));}function
expand($s){return
DIHelpers::expand($s,$this->parameters);}function&__get($name){if(!isset($this->registry[$name])){$this->getService($name);}return$this->registry[$name];}function
...
autowireArguments($class,$method,array$arguments){$rc=ClassReflection::from($class);if(!$rc->hasMethod($method)){if(!Validators::isList($arguments)){throw
new
...
DIHelpers::autowireArguments($rm,$arguments,$this);}function
...
DIStatement($def->class);}}elseif(!$def->factory){throw
new
ServiceCreationException("Class and factory are missing in service '$name' definition.");}}foreach($this->definitions
as$name=>$def){$factory=$this->normalizeEntity($this->expand($def->factory->entity));if(is_string($factory)&&preg_match('#^[\w\\\\]+\z#',$factory)&&$factory!==self::CREATED_SERVICE){if(!class_exists($factory)||!ClassReflection::from($factory)->isInstantiable()){throw
new
...
new
InvalidStateException("Class $def->class has not been found.");}$def->class=ClassReflection::from($def->class)->getName();if($def->autowired){foreach(class_parents($def->class)+class_implements($def->class)+array($def->class)as$parent){$this->classes[strtolower($parent)][]=(string)$name;}}}foreach($this->classes
as$class=>$foo){$this->addDependency(ClassReflection::from($class)->getFileName());}}private
function
...
Callback($factory);if(!$factory->isCallable()){throw
new
...
generateClass($parentClass='DIContainer'){unset($this->definitions[self::THIS_CONTAINER]);$this->addDefinition(self::THIS_CONTAINER)->setClass($parentClass);$this->prepareClassList();$class=new
PhpClassType('Container');$class->addExtend($parentClass);$class->addMethod('__construct')->addBody('parent::__construct(?);',array($this->expand($this->parameters)));$classes=$class->addProperty('classes',array());foreach($this->classes
...
as$name=>$def){if($def->shared){foreach($this->expand($def->tags)as$tag=>$value){$meta->value[$name][DIContainer::TAGS][$tag]=$value;}}}foreach($definitions
as$name=>$def){try{$name=(string)$name;$type=($tmp=$def->class)?$tmp:'object';$methodName=DIContainer::getMethodName($name,$def->shared);if(!PhpHelpers::isIdentifier($methodName)){throw
new
...
PhpLiteral('$'.end($v));}$code='$service = '.$this->formatStatement(DIHelpers::expand($def->factory,$parameters,TRUE)).";\n";$entity=$this->normalizeEntity($def->factory->entity);if($def->class&&$def->class!==$entity&&!$this->getServiceName($entity)){$code.=PhpHelpers::formatArgs("if (!\$service instanceof $def->class) {\n"."\tthrow new UnexpectedValueException(?);\n}\n",array("Unable to create service '$name', value returned by factory is not $def->class type."));}foreach((array)$def->setup
as$setup){$setup=DIHelpers::expand($setup,$parameters,TRUE);if(is_string($setup->entity)&&strpbrk($setup->entity,':@?')===FALSE){$setup->entity=array("@$name",$setup->entity);}$code.=$this->formatStatement($setup,$name).";\n";}return$code.='return $service;';}function
formatStatement(DIStatement$statement,$self=NULL){$entity=$this->normalizeEntity($statement->entity);$arguments=$statement->arguments;if(is_string($entity)&&Strings::contains($entity,'?')){return$this->formatPhp($entity,$arguments,$self);}elseif($service=$this->getServiceName($entity)){if($this->definitions[$service]->shared){if($arguments){throw
new
...
FunctionReflection(create_function(implode(', ',$params),''));$arguments=DIHelpers::autowireArguments($rm,$arguments,$this);return$this->formatPhp('$this->?(?*)',array(DIContainer::getMethodName($service,FALSE),$arguments),$self);}elseif($entity==='not'){return$this->formatPhp('!?',array($arguments[0]));}elseif(is_string($entity)){if($constructor=ClassReflection::from($entity)->getConstructor()){$this->addDependency($constructor->getFileName());$arguments=DIHelpers::autowireArguments($constructor,$arguments,$this);}elseif($arguments){throw
new
...
expand($value){return
DIHelpers::expand($value,$this->parameters,TRUE);}function
...
DIServiceDefinition){$tmp=array_keys($this->definitions,$entity[0],TRUE);$entity[0]="@$tmp[0]";}elseif($entity
instanceof
DIServiceDefinition){$tmp=array_keys($this->definitions,$entity,TRUE);$entity="@$tmp[0]";}elseif(is_array($entity)&&$entity[0]===$this){$entity[0]='@'.DIContainerBuilder::THIS_CONTAINER;}return$entity;}/**
...
__construct(DIContainer$container){if(PHP_VERSION_ID<50300){throw
new
...
<div class="nette-inner">
	...
	<div class="nette-ContainerPanel-parameters">
		<?php echo
DebugHelpers::clickableDump($this->container->parameters);?>
	</div>
...
DIContainer&&!$registry[$name]instanceof
DINestedAccessor):?>
					<?php echo
DebugHelpers::clickableDump($registry[$name],TRUE);?>
...
DebugHelpers::clickableDump($container->meta[$name],TRUE);}?></td>
		</tr>
		<?php endforeach?>
		</tbody>
	</table>
...
ob_get_clean();}private
function
getContainerProperty($name){$prop=ClassReflection::from('DIContainer')->getProperty($name);$prop->setAccessible(TRUE);return$prop->getValue($this->container);}}class
...
DIStatement){return
new
DIStatement(self::expand($var->entity,$params,$recursive),self::expand($var->arguments,$params,$recursive));}elseif(!is_string($var)){return$var;}$parts=preg_split('#%([\w.-]*)%#i',$var,-1,PREG_SPLIT_DELIM_CAPTURE);$res='';foreach($parts
as$n=>$part){if($n
%
2===0){$res.=$part;}elseif($part===''){$res.='%';}elseif(isset($recursive[$part])){throw
new
InvalidArgumentException('Circular reference detected for variables: '.implode(', ',array_keys($recursive)).'.');}else{$val=Arrays::get($params,explode('.',$part));if($recursive){$val=self::expand($val,$params,(is_array($recursive)?$recursive:array())+array($part=>1));}if(strlen($part)+2===strlen($var)){return$val;}if(!is_scalar($val)){throw
new
...
instanceof
DIContainerBuilder){$res[$num]='@'.$res[$num];}$optCount=0;}}elseif($parameter->isOptional()){$res[$num]=$parameter->isDefaultValueAvailable()?$parameter->getDefaultValue():NULL;$optCount++;}else{throw
new
...
DINestedAccessor
extends
Object{public$parameters;private$container;private$namespace;function
__construct(DIContainer$container,$namespace){$this->container=$container;$this->namespace=$namespace.'.';$this->parameters=&$container->parameters[$namespace];}function
__call($name,$args){if(substr($name,0,6)==='create'){return
call_user_func_array(array($this->container,DIContainer::getMethodName($this->namespace.substr($name,6),FALSE)),$args);}throw
new
...
DIServiceDefinition
extends
Object{public$class;public$factory;public$setup=array();public$parameters=array();public$tags=array();public$autowired=TRUE;public$shared=TRUE;public$internal=FALSE;function
...
DIStatement($factory,$args);return$this;}function
...
DIStatement($target,is_array($args)?$args:array_slice($_args,1));return$this;}function
...
DIStatement
extends
Object{public$entity;public$arguments;function
...
loadHttpData(){$path=explode('[',strtr(str_replace(array('[]',']'),'',$this->getHtmlName()),'.','_'));$this->setValue(Arrays::get($this->getForm()->getHttpData(),$path,NULL));}function
...
loadHttpData(){$path=$this->getHtmlName();$path=explode('[',strtr(str_replace(']','',strpos($path,'[')===FALSE?$path.'.x':substr($path,0,-2)),'.','_'));$this->setValue(Arrays::get($this->getForm()->getHttpData(),$path,NULL));}}class
SelectBox
extends
FormControl{private$items=array();protected$allowed=array();private$prompt=FALSE;private$useKeys=TRUE;function
...
getControl(){$selected=$this->getValue();$selected=is_array($selected)?array_flip($selected):array($selected=>TRUE);$control=parent::getControl();$option=Html::el('option');if($this->prompt!==FALSE){$control->add($this->prompt
instanceof
Html?$this->prompt->value(''):(string)$option->value('')->setText($this->translate((string)$this->prompt)));}foreach($this->items
...
instanceof
Html){$dest->add((string)$value2->value($key2)->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$dest->add((string)$option->value($key2)->selected(isset($selected[$key2]))->setText($this->translate((string)$value2)));}}}return$control;}}class
MultiSelectBox
extends
SelectBox{function
getValue(){return
...
Callback($filter);return$this;}function
sanitize($value){if($this->control->maxlength){$value=Strings::substring($value,0,$this->control->maxlength);}if(strcasecmp($this->control->getName(),'input')===0){$value=Strings::trim(strtr($value,"\r\n",'  '));}return$value;}function
...
Validators::isNumeric(self::filterFloat($control->getValue()));}static
function
...
instanceof
HiddenField&&!$control->getOption('rendered')){$s.=(string)$control->getControl();}}if(iterator_count($this->form->getComponents(TRUE,'TextInput'))<2){$s.='<!--[if IE]><input type=IEbug disabled style="display:none"><![endif]-->';}if($s){$s=$this->getWrapper('hidden container')->setHtml($s)."\n";}return$s.$this->form->getElementPrototype()->endTag()."\n";}function
...
trim(Strings::webalize($this->name,'.',FALSE),'.-');}function
getContentType(){if($this->isOk()&&$this->type===NULL){$this->type=MimeTypeDetector::fromFile($this->tmpName);}return$this->type;}function
...
Arrays::get($this->files,$_args,NULL);}final
function
...
new
HttpRequest($url,$query,$post,$files,$cookies,$headers,isset($_SERVER['REQUEST_METHOD'])?$_SERVER['REQUEST_METHOD']:NULL,isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:NULL,isset($_SERVER['REMOTE_HOST'])?$_SERVER['REMOTE_HOST']:NULL);}}final
class
HttpResponse
extends
Object
...
setExpiration($time){if(!$time){$this->setHeader('Cache-Control','s-maxage=0, max-age=0, must-revalidate');$this->setHeader('Expires','Mon, 23 Jan 1978 10:00:00 GMT');return$this;}$time=DateTime53::from($time);$this->setHeader('Cache-Control','max-age='.($time->format('U')-time()));$this->setHeader('Expires',self::date($time));return$this;}function
isSent(){return
...
function
date($time=NULL){$time=DateTime53::from($time);$time->setTimezone(new
...
new
InvalidStateException("session_start(): $error");}self::$started=TRUE;unset($_SESSION['__NT'],$_SESSION['__NS'],$_SESSION['__NM']);$nf=&$_SESSION['__NF'];@$nf['C']++;if(empty($nf['Time'])){$nf['Time']=time();$this->regenerated=TRUE;}$browserKey=$this->request->getCookie('nette-browser');if(!$browserKey){$browserKey=Strings::random();}$browserClosed=!isset($nf['B'])||$nf['B']!==$browserKey;$nf['B']=$browserKey;$this->sendCookie();if(isset($nf['META'])){$now=time();foreach($nf['META']as$section=>$metadata){if(is_array($metadata)){foreach($metadata
as$variable=>$value){if((!empty($value['B'])&&$browserClosed)||(!empty($value['T'])&&$now>$value['T'])||(isset($nf['DATA'][$section][$variable])&&is_object($nf['DATA'][$section][$variable])&&(isset($value['V'])?$value['V']:NULL)!=ClassReflection::from($nf['DATA'][$section][$variable])->getAnnotation('serializationVersion'))){if($variable===''){unset($nf['META'][$section],$nf['DATA'][$section]);continue
...
getSection($section,$class='SessionSection'){return
...
new
InvalidStateException("Unable to set 'session.$key' to value '$value' when session has been started".($this->started?".":" by session.auto_start or session_start()."));}if(isset($special[$key])){$key="session_$key";$key($value);}elseif(function_exists('ini_set')){ini_set("session.$key",$value);}elseif(!Framework::$iAmUsingBadHost){throw
new
...
setExpiration($time){if(empty($time)){return$this->setOptions(array('gc_maxlifetime'=>self::DEFAULT_FILE_LIFETIME,'cookie_lifetime'=>0));}else{$time=DateTime53::from($time)->format('U')-time();return$this->setOptions(array('gc_maxlifetime'=>$time,'cookie_lifetime'=>$time));}}function
...
function
sendCookie(){$cookie=$this->getCookieParameters();$this->response->setCookie(session_name(),session_id(),$cookie['lifetime']?$cookie['lifetime']+time():0,$cookie['path'],$cookie['domain'],$cookie['secure'],$cookie['httponly'])->setCookie('nette-browser',$_SESSION['__NF']['B'],HttpResponse::BROWSER,$cookie['path'],$cookie['domain']);}}class
SessionSection
extends
Object
implements
...
Callback($callback);$this->childrenCallback=$childrenCallback===NULL?NULL:new
Callback($childrenCallback);}function
...
LatteCompiler
extends
Object{public$defaultContentType=self::CONTENT_XHTML;private$tokens;private$output;private$position;private$macros;private$macroHandlers;private$htmlNodes=array();private$macroNodes=array();private$attrCodes=array();private$contentType;private$context;private$templateId;const
...
as$this->position=>$token){if($token->type===LatteToken::TEXT){$this->output.=$token->text;}elseif($token->type===LatteToken::MACRO_TAG){$isRightmost=!isset($tokens[$this->position+1])||substr($tokens[$this->position+1]->text,0,1)==="\n";$this->writeMacro($token->name,$token->value,$token->modifiers,$isRightmost);}elseif($token->type===LatteToken::HTML_TAG_BEGIN){$this->processHtmlTagBegin($token);}elseif($token->type===LatteToken::HTML_TAG_END){$this->processHtmlTagEnd($token);}elseif($token->type===LatteToken::HTML_ATTRIBUTE){$this->processHtmlAttribute($token);}elseif($token->type===LatteToken::COMMENT){$this->processComment($token);}}}catch(CompileException$e){$e->sourceLine=$token->line;throw$e;}foreach($this->htmlNodes
...
function
processHtmlTagBegin(LatteToken$token){if($token->closing){do{$htmlNode=array_pop($this->htmlNodes);if(!$htmlNode){$htmlNode=new
...
function
processComment(LatteToken$token){$isLeftmost=trim(substr($this->output,strrpos("\n$this->output","\n")))==='';if(!$isLeftmost){$this->output.=substr($token->text,strlen(rtrim($token->text,"\n")));}}function
...
new
CompileException("Unknown macro-attribute ".Parser::N_PREFIX.implode(' and '.Parser::N_PREFIX,array_keys($attrs)));}if(!$htmlNode->closing){$htmlNode->attrCode=&$this->attrCodes[$uniq=' n:'.Strings::random()];$code=substr_replace($code,$uniq,($tmp=strrpos($code,'/>'))?$tmp:strrpos($code,'>'),0);}foreach($left
...
LatteFilter
extends
Object{private$parser;private$compiler;function
__construct(){$this->parser=new
Parser;$this->compiler=new
LatteCompiler;$this->compiler->defaultContentType=LatteCompiler::CONTENT_XHTML;CoreMacros::install($this->compiler);$this->compiler->addMacro('cache',new
CacheMacro($this->compiler));UIMacros::install($this->compiler);FormMacros::install($this->compiler);}function
...
HtmlNode
extends
Object{public$name;public$isEmpty=FALSE;public$attrs=array();public$macroAttrs=array();public$closing=FALSE;public$attrCode;public$offset;function
...
MacroTokenizer($this->args);$this->data=new
...
array('CacheMacro::initRuntime($template, $_g);');}}function
nodeOpened(MacroNode$node){$this->used=TRUE;$node->isEmpty=FALSE;$node->openingCode=PhpWriter::using($node)->write('<?php if (CacheMacro::createCache($netteCacheStorage, %var, $_g->caches, %node.array?)) { ?>',Strings::random());}function
nodeClosed(MacroNode$node){$node->closingCode='<?php $_l->tmp = array_pop($_g->caches); if (!$_l->tmp instanceof stdClass) $_l->tmp->end(); } ?>';}static
function
initRuntime(FileTemplate$template,stdClass$global){if(!empty($global->caches)){end($global->caches)->dependencies[Cache::FILES][]=$template->getFile();}}static
function
...
stdClass;}$key=array_merge(array($key),array_intersect_key($args,range(0,count($args))));}if($parents){end($parents)->dependencies[Cache::ITEMS][]=$key;}$cache=new
Cache($cacheStorage,'Nette.Templating.Cache');if($helper=$cache->start($key)){if(isset($args['expire'])){$args['expiration']=$args['expire'];}$helper->dependencies=array(Cache::TAGS=>isset($args['tags'])?$args['tags']:NULL,Cache::EXPIRATION=>isset($args['expiration'])?$args['expiration']:'+ 7 days');$parents[]=$helper;}return$helper;}}class
MacroSet
extends
Object
implements
IMacro{private$compiler;private$macros;function
__construct(LatteCompiler$compiler){$this->compiler=$compiler;}function
...
function
install(LatteCompiler$compiler){return
new
...
function
compile(MacroNode$node,$def){$node->tokenizer->reset();$writer=PhpWriter::using($node,$this->compiler);if(is_string($def)&&substr($def,0,1)!=="\0"){return$writer->write($def);}else{return
Callback::create($def)->invoke($node,$writer);}}function
getCompiler(){return$this->compiler;}}class
CoreMacros
extends
MacroSet{static
function
install(LatteCompiler$compiler){$me=new
...
finalize(){return
array('list($_l, $_g) = CoreMacros::initRuntime($template, '.var_export($this->getCompiler()->getTemplateId(),TRUE).')');}function
macroIf(MacroNode$node,PhpWriter$writer){if($node->data->capture=($node->args==='')){return'ob_start()';}if($node->prefix===MacroNode::PREFIX_TAG){return$writer->write($node->htmlNode->closing?'if (array_pop($_l->ifs)):':'if ($_l->ifs[] = (%node.args)):');}return$writer->write('if (%node.args):');}function
...
macroInclude(MacroNode$node,PhpWriter$writer){$code=$writer->write('CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParameters(), $_l->templates[%var])',$this->getCompiler()->getTemplateId());if($node->modifiers){return$writer->write('echo %modify(%raw->__toString(TRUE))',$code);}else{return$code.'->render()';}}function
macroUse(MacroNode$node,PhpWriter$writer){Callback::create($node->tokenizer->fetchWord(),'install')->invoke($this->getCompiler())->initialize();}function
...
macroAttr(MacroNode$node,PhpWriter$writer){return$writer->write('echo Html::el(NULL, %node.array)->attributes()');}function
macroOldAttr(MacroNode$node){return
Strings::replace($node->args.' ','#\)\s+#',')->');}function
macroDump(MacroNode$node,PhpWriter$writer){$args=$writer->formatArgs();return'Debugger::barDump('.($node->args?"array(".$writer->write('%var',$args)." => $args)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))';}function
...
macroVar(MacroNode$node,PhpWriter$writer){$out='';$var=TRUE;$tokenizer=$writer->preprocess();while($token=$tokenizer->fetchToken()){if($var&&($token['type']===MacroTokenizer::T_SYMBOL||$token['type']===MacroTokenizer::T_VARIABLE)){if($node->name==='default'){$out.="'".ltrim($token['value'],"$")."'";}else{$out.='$'.ltrim($token['value'],"$");}$var=NULL;}elseif(($token['value']==='='||$token['value']==='=>')&&$token['depth']===0){$out.=$node->name==='default'?'=>':'=';$var=FALSE;}elseif($token['value']===','&&$token['depth']===0){$out.=$node->name==='default'?',':';';$var=TRUE;}elseif($var===NULL&&$node->name==='default'&&$token['type']!==MacroTokenizer::T_WHITESPACE){throw
new
...
function
renderFormEnd(Form$form){$s='';if(strcasecmp($form->getMethod(),'get')===0){$url=explode('?',$form->getElementPrototype()->action,2);if(isset($url[1])){list($url[1])=explode('#',$url[1],2);foreach(preg_split('#[;&]#',$url[1])as$param){$parts=explode('=',$param,2);$name=urldecode($parts[0]);if(!isset($form[$name])){$s.=Html::el('input',array('type'=>'hidden','name'=>$name,'value'=>urldecode($parts[1])));}}}}foreach($form->getComponents(TRUE,'HiddenField')as$control){if(!$control->getOption('rendered')){$s.=$control->getControl();}}if(iterator_count($form->getComponents(TRUE,'TextInput'))<2){$s.='<!--[if IE]><input type=IEbug disabled style="display:none"><![endif]-->';}echo($s?"<div>$s</div>\n":'').$form->getElementPrototype()->endTag()."\n";}}class
UIMacros
extends
MacroSet{private$namedBlocks=array();private$extends;static
function
install(LatteCompiler$compiler){$me=new
self($compiler);$me->addMacro('include',array($me,'macroInclude'));$me->addMacro('includeblock',array($me,'macroIncludeBlock'));$me->addMacro('extends',array($me,'macroExtends'));$me->addMacro('layout',array($me,'macroExtends'));$me->addMacro('block',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('define',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('snippet',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('ifset',array($me,'macroIfset'),'endif');$me->addMacro('widget',array($me,'macroControl'));$me->addMacro('control',array($me,'macroControl'));$me->addMacro('href',NULL,NULL,create_function('MacroNode $node, PhpWriter $writer','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('me'=>$me)).'-1], EXTR_REFS);
...
macroIfCurrent(MacroNode$node,PhpWriter$writer){return$writer->write(($node->args?'try { $_presenter->link(%node.word, %node.array?); } catch (InvalidLinkException $e) {}':'').'; if ($_presenter->getLastCreatedRequestFlag("current")):');}function
macroContentType(MacroNode$node,PhpWriter$writer){if(Strings::contains($node->args,'xhtml')){$this->getCompiler()->setContentType(LatteCompiler::CONTENT_XHTML);}elseif(Strings::contains($node->args,'html')){$this->getCompiler()->setContentType(LatteCompiler::CONTENT_HTML);}elseif(Strings::contains($node->args,'xml')){$this->getCompiler()->setContentType(LatteCompiler::CONTENT_XML);}elseif(Strings::contains($node->args,'javascript')){$this->getCompiler()->setContentType(LatteCompiler::CONTENT_JS);}elseif(Strings::contains($node->args,'css')){$this->getCompiler()->setContentType(LatteCompiler::CONTENT_CSS);}elseif(Strings::contains($node->args,'calendar')){$this->getCompiler()->setContentType(LatteCompiler::CONTENT_ICAL);}else{$this->getCompiler()->setContentType(LatteCompiler::CONTENT_TEXT);}if(Strings::contains($node->args,'/')){return$writer->write('$netteHttpResponse->setHeader("Content-Type", %var)',$node->args);}}function
...
function
renderSnippets(Control$control,stdClass$local,array$params){$control->snippetMode=FALSE;$payload=$control->getPresenter()->getPayload();if(isset($local->blocks)){foreach($local->blocks
...
Tokenizer
extends
Object{public$tokens;public$position=0;public$ignored=array();private$input;private$re;private$types;public$current;function
...
new
InvalidArgumentException('Template is not valid UTF-8 stream.');}$input=str_replace("\r\n","\n",$input);$this->input=$input;$this->output=array();$this->offset=0;$this->setSyntax($this->defaultSyntax);$this->setContext(self::CONTEXT_TEXT);$this->lastHtmlTag=$this->syntaxEndTag=NULL;while($this->offset<strlen($input)){$matches=$this->{"context".$this->context[0]}();if(!$matches){break;}elseif(!empty($matches['comment'])){$this->addToken(LatteToken::COMMENT,$matches[0]);}elseif(!empty($matches['macro'])){$token=$this->addToken(LatteToken::MACRO_TAG,$matches[0]);list($token->name,$token->value,$token->modifiers)=$this->parseMacroTag($matches['macro']);}$this->filter();}if($this->offset<strlen($input)){$this->addToken(LatteToken::TEXT,substr($this->input,$this->offset));}return$this->output;}private
function
contextText(){$matches=$this->match('~
			(?:(?<=\n|^)[ \t]*)?<(?P<closing>/?)(?P<tag>[a-z0-9:]+)|  ##  begin of HTML tag <tag </tag - ignores <!DOCTYPE
			<(?P<htmlcomment>!--)|     ##  begin of HTML comment <!--
			'.$this->macroRe.'         ##  macro tag
		~xsi');if(!empty($matches['htmlcomment'])){$this->addToken(LatteToken::HTML_TAG_BEGIN,$matches[0]);$this->setContext(self::CONTEXT_COMMENT);}elseif(!empty($matches['tag'])){$token=$this->addToken(LatteToken::HTML_TAG_BEGIN,$matches[0]);$token->name=$matches['tag'];$token->closing=(bool)$matches['closing'];$this->lastHtmlTag=$matches['closing'].strtolower($matches['tag']);$this->setContext(self::CONTEXT_TAG);}return$matches;}private
function
contextCData(){$matches=$this->match('~
			</(?P<tag>'.$this->lastHtmlTag.')(?![a-z0-9:])| ##  end HTML tag </tag
			'.$this->macroRe.'              ##  macro tag
		~xsi');if(!empty($matches['tag'])){$token=$this->addToken(LatteToken::HTML_TAG_BEGIN,$matches[0]);$token->name=$this->lastHtmlTag;$token->closing=TRUE;$this->lastHtmlTag='/'.$this->lastHtmlTag;$this->setContext(self::CONTEXT_TAG);}return$matches;}private
function
contextTag(){$matches=$this->match('~
			(?P<end>\ ?/?>)([ \t]*\n)?|  ##  end of HTML tag
			'.$this->macroRe.'|          ##  macro tag
			\s*(?P<attr>[^\s/>={]+)(?:\s*=\s*(?P<value>["\']|[^\s/>{]+))? ## begin of HTML attribute
		~xsi');if(!empty($matches['end'])){$this->addToken(LatteToken::HTML_TAG_END,$matches[0]);$this->setContext(!$this->xmlMode&&in_array($this->lastHtmlTag,array('script','style'))?self::CONTEXT_CDATA:self::CONTEXT_TEXT);}elseif(isset($matches['attr'])&&$matches['attr']!==''){$token=$this->addToken(LatteToken::HTML_ATTRIBUTE,$matches[0]);$token->name=$matches['attr'];$token->value=isset($matches['value'])?$matches['value']:'';if($token->value==='"'||$token->value==="'"){if(Strings::startsWith($token->name,self::N_PREFIX)){$token->value='';if($m=$this->match('~(.*?)'.$matches['value'].'~xsi')){$token->value=$m[1];$token->text.=$m[0];}}else{$this->setContext(self::CONTEXT_ATTRIBUTE,$matches['value']);}}}return$matches;}private
function
contextAttribute(){$matches=$this->match('~
			(?P<quote>'.$this->context[1].')|  ##  end of HTML attribute
			'.$this->macroRe.'                 ##  macro tag
		~xsi');if(!empty($matches['quote'])){$this->addToken(LatteToken::TEXT,$matches[0]);$this->setContext(self::CONTEXT_TAG);}return$matches;}private
function
contextComment(){$matches=$this->match('~
			(?P<htmlcomment>--\s*>)|   ##  end of HTML comment
			'.$this->macroRe.'         ##  macro tag
		~xsi');if(!empty($matches['htmlcomment'])){$this->addToken(LatteToken::HTML_TAG_END,$matches[0]);$this->setContext(self::CONTEXT_TEXT);}return$matches;}private
function
contextNone(){$matches=$this->match('~
			'.$this->macroRe.'     ##  macro tag
		~xsi');return$matches;}private
function
match($re){if($matches=Strings::match($this->input,$re,PREG_OFFSET_CAPTURE,$this->offset)){$value=substr($this->input,$this->offset,$matches[0][1]-$this->offset);if($value!==''){$this->addToken(LatteToken::TEXT,$value);}$this->offset=$matches[0][1]+strlen($matches[0][0]);foreach($matches
...
LatteToken;$token->type=$type;$token->text=$text;$token->line=substr_count($this->input,"\n",0,max(1,$this->offset-1))+1;return$token;}protected
function
filter(){$token=end($this->output);if($token->type===LatteToken::MACRO_TAG&&$token->name==='/syntax'){$this->setSyntax($this->defaultSyntax);$token->type=LatteToken::COMMENT;}elseif($token->type===LatteToken::MACRO_TAG&&$token->name==='syntax'){$this->setSyntax($token->value);$token->type=LatteToken::COMMENT;}elseif($token->type===LatteToken::HTML_ATTRIBUTE&&$token->name==='n:syntax'){$this->setSyntax($token->value);$this->syntaxEndTag='/'.$this->lastHtmlTag;$token->type=LatteToken::COMMENT;}elseif($token->type===LatteToken::HTML_TAG_END&&$this->lastHtmlTag===$this->syntaxEndTag){$this->setSyntax($this->defaultSyntax);}elseif($token->type===LatteToken::MACRO_TAG&&$token->name==='contentType'){if(preg_match('#html|xml#',$token->value,$m)){$this->xmlMode=$m[0]==='xml';$this->setContext(self::CONTEXT_TEXT);}else{$this->setContext(self::CONTEXT_NONE);}}}}class
PhpWriter
extends
Object{private$argsTokenizer;private$modifiers;private$compiler;static
function
using(MacroNode$node,LatteCompiler$compiler=NULL){return
new
self($node->tokenizer,$node->modifiers,$compiler);}function
__construct(MacroTokenizer$argsTokenizer,$modifiers=NULL,LatteCompiler$compiler=NULL){$this->argsTokenizer=$argsTokenizer;$this->modifiers=$modifiers;$this->compiler=$compiler;}function
write($mask){$args=func_get_args();array_shift($args);$word=strpos($mask,'%node.word')===FALSE?NULL:$this->argsTokenizer->fetchWord();$me=$this;$mask=Strings::replace($mask,'#%escape(\(([^()]*+|(?1))+\))#',create_function('$m','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('me'=>$me)).'-1], EXTR_REFS);
			return $me->escape(substr($m[1], 1, -1));
		'));$mask=Strings::replace($mask,'#%modify(\(([^()]*+|(?1))+\))#',create_function('$m','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('me'=>$me)).'-1], EXTR_REFS);
			return $me->formatModifiers(substr($m[1], 1, -1));
		'));return
Strings::replace($mask,'#([,+]\s*)?%(node\.word|node\.array|node\.args|var|raw)(\?)?(\s*\+\s*)?()#',create_function('$m','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('me'=>$me,'word'=>$word,'args'=>&$args)).'-1], EXTR_REFS);
...
LatteCompiler::CONTENT_XHTML:case
LatteCompiler::CONTENT_HTML:$context=$this->compiler->getContext();switch($context[0]){case
LatteCompiler::CONTEXT_SINGLE_QUOTED:case
LatteCompiler::CONTEXT_DOUBLE_QUOTED:if($context[1]===LatteCompiler::CONTENT_JS){$s="TemplateHelpers::escapeJs($s)";}elseif($context[1]===LatteCompiler::CONTENT_CSS){$s="TemplateHelpers::escapeCss($s)";}$quote=$context[0]===LatteCompiler::CONTEXT_DOUBLE_QUOTED?'':', ENT_QUOTES';return"htmlSpecialChars($s$quote)";case
LatteCompiler::CONTEXT_COMMENT:return"TemplateHelpers::escapeHtmlComment($s)";case
LatteCompiler::CONTENT_JS:case
LatteCompiler::CONTENT_CSS:return'TemplateHelpers::escape'.ucfirst($context[0])."($s)";default:return"TemplateHelpers::escapeHtml($s, ENT_NOQUOTES)";}case
LatteCompiler::CONTENT_XML:case
LatteCompiler::CONTENT_JS:case
LatteCompiler::CONTENT_CSS:case
LatteCompiler::CONTENT_ICAL:return'TemplateHelpers::escape'.ucfirst($this->compiler->getContentType())."($s)";case
LatteCompiler::CONTENT_TEXT:return$s;default:return"\$template->escape($s)";}}}class
LatteToken
extends
Object{const
...
Callback($this,'_rebuildCallback'));parent::register();return$this;}function
tryLoad($type){$type=ltrim(strtolower($type),'\\');$info=&$this->classes[$type];if(isset($this->missing[$type])||(is_int($info)&&$info>=self::RETRY_LIMIT)){return;}if($this->autoRebuild){if(!is_array($info)||!is_file($info['file'])){$info=is_int($info)?$info+1:0;if($this->rebuilt){$this->getCache()->save($this->getKey(),$this->classes,array(Cache::CONSTS=>'Framework::REVISION'));}else{$this->rebuild();}}elseif(!$this->rebuilt&&filemtime($info['file'])!==$info['time']){$this->updateFile($info['file']);if(!isset($this->classes[$type])){$this->classes[$type]=0;}$this->getCache()->save($this->getKey(),$this->classes,array(Cache::CONSTS=>'Framework::REVISION'));}}if(isset($this->classes[$type]['file'])){LimitedScope::load($this->classes[$type]['file'],TRUE);self::$count++;}else{$this->missing[$type]=TRUE;}}function
...
Callback($this,'_rebuildCallback'));}function
...
InvalidStateException("Ambiguous class $class resolution; defined in {$info['file']} and in $file.");if(PHP_VERSION_ID<50300){Debugger::_exceptionHandler($e);exit;}else{throw$e;}}$info=array('file'=>$file,'time'=>filemtime($file),'orig'=>$class);}}}$dp=array(Cache::CONSTS=>'Framework::REVISION');$this->classes+=$missing;return$this->classes;}private
function
...
as$item){if($item=realpath($item)){$disallow[$item]=TRUE;}}$iterator=Finder::findFiles(is_array($this->acceptFiles)?$this->acceptFiles:preg_split('#[,\s]+#',$this->acceptFiles))->filter(create_function('$file','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('disallow'=>&$disallow)).'-1], EXTR_REFS);
...
InvalidStateException("Ambiguous class $class resolution; defined in {$info['file']} and in $file.");if(PHP_VERSION_ID<50300){Debugger::_exceptionHandler($e);exit;}else{throw$e;}}$info=array('file'=>$file,'time'=>filemtime($file),'orig'=>$class);}}}private
function
...
as$email=>$recipient){if($recipient!==NULL&&!Strings::checkEncoding($recipient)){Validators::assert($recipient,'unicode',"header '$name'");}if(preg_match('#[\r\n]#',$recipient)){throw
new
InvalidArgumentException("Name must not contain line separator.");}Validators::assert($email,'email',"header '$name'");$tmp[$email]=$recipient;}}else{$value=(string)$value;if(!Strings::checkEncoding($value)){throw
new
...
new
FileNotFoundException("Unable to read file '$file'.");}}else{$content=(string)$content;}$part->setBody($content);$part->setContentType($contentType?$contentType:MimeTypeDetector::fromString($content));$part->setEncoding(preg_match('#(multipart|message)/#A',$contentType)?self::ENCODING_8BIT:self::ENCODING_BASE64);$part->setHeader('Content-Disposition',$disposition.'; filename="'.Strings::fixEncoding(basename($file)).'"');return$part;}function
...
instanceof
ITemplate){$text->mail=$this;$this->setBody($text->__toString(TRUE));}elseif($text==NULL&&$this->html!=NULL){$text=Strings::replace($this->html,array('#<(style|script|head).*</\\1>#Uis'=>'','#<t[dh][ >]#i'=>" $0",'#[\r\n]+#'=>' ','#<(/?p|/?h\d|li|br|/tr)[ >/]#i'=>"\n$0"));$text=html_entity_decode(strip_tags($text),ENT_QUOTES,'UTF-8');$text=Strings::replace($text,'#[ \t]+#',' ');$this->setBody(trim($text));}}private
function
...
self::$cache[$type][$member];}if(self::$useReflection===NULL){self::$useReflection=(bool)ClassReflection::from(__CLASS__)->getDocComment();}if(self::$useReflection){$annotations=self::parseComment($r->getDocComment());}else{if(!self::$cacheStorage){self::$cacheStorage=new
DevNullStorage;}$outerCache=new
Cache(self::$cacheStorage,'Nette.Reflection.Annotations');if(self::$cache===NULL){self::$cache=(array)$outerCache->offsetGet('list');self::$timestamps=isset(self::$cache['*'])?self::$cache['*']:array();}if(!isset(self::$cache[$type])&&$file){self::$cache['*'][$file]=filemtime($file);self::parseScript($file);$outerCache->save('list',self::$cache);}if(isset(self::$cache[$type][$member])){$annotations=self::$cache[$type][$member];}else{$annotations=array();}}if($r
instanceof
...
__construct(User$user){$this->user=$user;}function
getTab(){ob_start();?>
...
DebugHelpers::clickableDump($this->user->getIdentity());else:?><p>no identity</p><?php endif?>
</div>
<?php
return
ob_get_clean();}}class
...
Callback($assertion):NULL;if($toAdd){foreach($resources
...
User
extends
Object{public$guestRole='guest';public$authenticatedRole='authenticated';public$onLoggedIn;public$onLoggedOut;private$storage;private$authenticator;private$authorizator;private$context;function
__construct(IUserStorage$storage,DIContainer$context){$this->storage=$storage;$this->context=$context;}final
function
getStorage(){return$this->storage;}function
...
getSource(){return$this->source;}function
render(){$cache=new
Cache($storage=$this->getCacheStorage(),'Nette.Template');$cached=$compiled=$cache->load($this->source);if($compiled===NULL){$compiled=$this->compile();$cache->save($this->source,$compiled,array(Cache::CONSTS=>'Framework::REVISION'));$cached=$cache->load($this->source);}if($cached!==NULL&&$storage
instanceof
PhpFileStorage){LimitedScope::load($cached['file'],$this->getParameters());}else{LimitedScope::evaluate($compiled,$this->getParameters());}}function
...
TemplateHelpers::optimizePhp($code);}function
registerFilter($callback){$callback=new
Callback($callback);if(in_array($callback,$this->filters)){throw
new
...
Callback($callback);return$this;}function
registerHelperLoader($callback){$this->helperLoaders[]=new
Callback($callback);return$this;}final
function
...
new
DevNullStorage;}return$this->cacheStorage;}private
static
function
...
instanceof
PhpFileStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$compiled=$cache->load($this->file);if($compiled===NULL){try{$compiled="<?php\n\n// source file: $this->file\n\n?>".$this->compile();}catch(TemplateException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$compiled,array(Cache::FILES=>$this->file,Cache::CONSTS=>'Framework::REVISION'));$cached=$cache->load($this->file);}if($cached!==NULL&&$storage
instanceof
PhpFileStorage){LimitedScope::load($cached['file'],$this->getParameters());}else{LimitedScope::evaluate($compiled,$this->getParameters());}}}final
class
TemplateHelpers{private
static$helpers=array('normalize'=>'Strings::normalize','toascii'=>'Strings::toAscii','webalize'=>'Strings::webalize','truncate'=>'Strings::truncate','lower'=>'Strings::lower','upper'=>'Strings::upper','firstupper'=>'Strings::firstUpper','capitalize'=>'Strings::capitalize','trim'=>'Strings::trim','padleft'=>'Strings::padLeft','padright'=>'Strings::padRight','reverse'=>'Strings::reverse','replacere'=>'Strings::replace','url'=>'rawurlencode','striptags'=>'strip_tags','substr'=>'Strings::substring','repeat'=>'str_repeat','implode'=>'implode','number'=>'number_format');public
...
function
strip($s){return
Strings::replace($s,'#(</textarea|</pre|</script|^).*?(?=<textarea|<pre|<script|\z)#si',create_function('$m','
				return trim(preg_replace(\'#[ \\t\\r\\n]+#\', " ", $m[0]));
			'));}static
function
indent($s,$level=1,$chars="\t"){if($level>=1){$s=Strings::replace($s,'#<(textarea|pre).*?</\\1#si',create_function('$m','
				return strtr($m[0], " \\t\\r\\n", "\\x1F\\x1E\\x1D\\x1A");
			'));$s=Strings::indent($s,$level,$chars);$s=strtr($s,"\x1F\x1E\x1D\x1A"," \t\r\n");}return$s;}static
function
date($time,$format=NULL){if($time==NULL){return
NULL;}if(!isset($format)){$format=self::$dateFormat;}$time=DateTime53::from($time);return
...
function
dataStream($data,$type=NULL){if($type===NULL){$type=MimeTypeDetector::fromString($data);}return'data:'.($type?"$type;":'').'base64,'.base64_encode($data);}static
function
...
NRecursiveCallbackFilterIterator($iterator,create_function('$file','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('filters'=>$filters)).'-1], EXTR_REFS);
...
NCallbackFilterIterator($iterator,create_function('$file','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('groups'=>$groups)).'-1], EXTR_REFS);
...
new
InvalidArgumentException('Invalid date predicate format.');}list(,$operator,$date)=$matches;$operator=$operator?$operator:'=';}$date=DateTime53::from($date)->format('U');return$this->filter(create_function('$file','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('operator'=>$operator,'date'=>$date)).'-1], EXTR_REFS);
...
new
JsonException(isset(self::$messages[$error])?self::$messages[$error]:'Unknown error',$error);}$json=str_replace(array("\xe2\x80\xa8","\xe2\x80\xa9"),array('\u2028','\u2029'),$json);return$json;}static
function
...
DateTime53($t);}else{$value=$t;}$hasValue=TRUE;}}if($inlineParser){if($hasKey||$hasValue){$this->addValue($result,$hasKey,$key,$hasValue?$value:NULL);}}else{if($hasValue&&!$hasKey){if($result===NULL){$result=$value;}else{$this->error();}}elseif($hasKey){$this->addValue($result,$key!==NULL,$key,$hasValue?$value:NULL);}}return$result;}private
function
...
PhpClassType
extends
Object{public$name;public$type='class';public$final;public$abstract;public$extends=array();public$implements=array();public$traits=array();public$documents=array();public$consts=array();public$properties=array();public$methods=array();function
...
PhpProperty;return$this->properties[$name]=$property->setName($name)->setValue($value);}function
addMethod($name){$method=new
PhpMethod;if($this->type==='interface'){$method->setVisibility('')->setBody(FALSE);}else{$method->setVisibility('public');}return$this->methods[$name]=$method->setName($name);}function
__call($name,$args){return
ObjectMixin::callProperty($this,$name,$args);}function
__toString(){$consts=array();foreach($this->consts
as$name=>$value){$consts[]="const $name = ".PhpHelpers::dump($value).";\n";}$properties=array();foreach($this->properties
...
PhpMethod
extends
Object{public$name;public$parameters=array();public$uses=array();public$body;public$static;public$visibility;public$final;public$abstract;public$returnReference;public$documents=array();function
addParameter($name,$defaultValue=NULL){$param=new
PhpParameter;if(func_num_args()>1){$param->setOptional(TRUE)->setDefaultValue($defaultValue);}return$this->parameters[]=$param->setName($name);}function
addUse($name){$param=new
PhpParameter;return$this->uses[]=$param->setName($name);}function
setBody($statement,array$args=NULL){$this->body=func_num_args()>1?PhpHelpers::formatArgs($statement,$args):$statement;return$this;}function
addBody($statement,array$args=NULL){$this->body.=(func_num_args()>1?PhpHelpers::formatArgs($statement,$args):$statement)."\n";return$this;}function
__call($name,$args){return
ObjectMixin::callProperty($this,$name,$args);}function
__toString(){$parameters=array();foreach($this->parameters
as$param){$parameters[]=($param->typeHint?$param->typeHint.' ':'').($param->reference?'&':'').'$'.$param->name.($param->optional?' = '.PhpHelpers::dump($param->defaultValue):'');}$uses=array();foreach($this->uses
...
PhpParameter
extends
Object{public$name;public$reference;public$typeHint;public$optional;public$defaultValue;function
__call($name,$args){return
ObjectMixin::callProperty($this,$name,$args);}}class
PhpLiteral{public$value='';function
__construct($value){$this->value=(string)$value;}}class
PhpProperty
extends
Object{public$name;public$value;public$static;public$visibility='public';public$documents=array();function
__call($name,$args){return
ObjectMixin::callProperty($this,$name,$args);}}class
Strings{final
function
...
random($length=10,$charlist='0-9a-z'){$charlist=str_shuffle(preg_replace_callback('#.-.#',create_function('$m','
		...
		'),$charlist));$chLen=strlen($charlist);static$rand3;if(!$rand3){$rand3=md5(serialize($_SERVER),TRUE);}$s='';for($i=0;$i<$length;$i++){if($i
%
...
array();}set_error_handler(create_function('$severity, $message','extract($GLOBALS[0]['.array_push($GLOBALS[0],array('pattern'=>$pattern)).'-1], EXTR_REFS); // preg_last_error does not return compile errors
			restore_error_handler();
			throw new RegexpException("$message in pattern: $pattern");
		'));$res=preg_match_all($pattern,$subject,$m,($flags&PREG_PATTERN_ORDER)?$flags:($flags|PREG_SET_ORDER),$offset);restore_error_handler();if(preg_last_error()){throw
new
...
static$counters=array('string'=>'strlen','unicode'=>array('Strings','length'),'array'=>'count','list'=>'count','alnum'=>'strlen','alpha'=>'strlen','digit'=>'strlen','lower'=>'strlen','space'=>'strlen','upper'=>'strlen','xdigit'=>'strlen');static
function
...
new
AssertionException('Missing '.str_replace('%',$field,$label).'.');}elseif($expected){self::assert($arr[$field],$expected,str_replace('%',$field,$label));}}static
function
...
AssertionException
extends
Exception{}Debugger::_init();SafeStream::register();function
